...
public class GraphLink<E extends Comparable<E>> {
    ...
    private Vertex<E> findParent(ArrayList<Vertex<E>> parentMap, Vertex<E> node) {
        // Esto es una simplificación. En una implementación real, deberíamos llevar un registro de padres.
        // Aquí asumimos que el padre está antes en la lista (no es óptimo, pero funciona para el ejemplo).
        int index = parentMap.indexOf(node);
        if (index <= 0) return null; // No tiene padre o es el inicio
